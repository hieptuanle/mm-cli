...
## "Here's a Mattermost link, what's in it?"

The user pastes a permalink. `mm` accepts the URL directly:
...
You don't have to extract the ID — the CLI does it for you.
...
The user wants to respond to something in chat. `mm` is read-only — you can't send messages — but you can help:
...
mm user @person-they-are-replying-to
```
...
When a result needs to flow into a script or another command, pick the right format:

```bash
# Programmatic: NDJSON, one record per line
mm mentions --since 1d --ndjson | jq -r '"\(.created_at) \(.author): \(.message)"'

# Quote into another chat: --raw avoids ANSI escapes
mm thread <thread_id> --raw | pbcopy
```
