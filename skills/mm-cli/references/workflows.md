...
Common multi-step patterns using `mm`. Default output is colored human; add `--json` for machine output.
...
## Following a permalink someone sent

The user pastes a Mattermost link. `mm` accepts it directly:

```bash
mm thread https://chat.example.com/<team>/pl/<post-id>
mm messages https://chat.example.com/<team>/channels/<name>
```

No need to extract the ID by hand — the CLI parses the URL.
...
Use `--ndjson` so each post is a line, then pipe into `jq`/`fzf`/`rg`:

```bash
# Top mentions, just author + message
mm mentions --since 1d --ndjson | jq -r '"\(.author): \(.message)"'

# Unread channels with > 100 unread
mm unread --ndjson | jq 'select(.unread > 100)'
```
...
| `yesterday` | Since midnight UTC the day before |
| `2025-03-05` | Since a specific date |
| `2025-03-05T14:30` | Since an ISO timestamp |
| `@1741171200` | Since a Unix epoch (seconds) |
| `1741171200000` | Since a raw Unix epoch (milliseconds) |
